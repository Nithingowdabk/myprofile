  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-3 gap-12 items-start"> {/* Changed to 3 columns and align items to start */}
          <motion.div
            initial={{ x: -100, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ delay: 0.5 }}
            className="md:col-span-1 flex justify-center md:justify-start" // Image column
          >
            <img 
              src={profileImage} 
              alt="Profile" 
              className="rounded-lg shadow-lg w-64 h-64 object-cover" // Added styling for the image
            />
          </motion.div>
          <motion.div
            initial={{ x: -100, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ delay: 0.5 }}
            className="space-y-6 md:col-span-1" // Text content column
          >
